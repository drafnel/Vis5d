...
#define gimp_fatal_error printf
...
static void
_hsv_to_rgb (gdouble  h, gdouble  s, gdouble  v,
...
static void
_rgb_to_hsv (gdouble  r, gdouble  g, gdouble  b,
  ...
  *a = seg->a0 + (seg->a1 - seg->a0) * factor;
  ...
  if (seg->color == GRAD_RGB)
  ...
  else
    {
      ...
      _rgb_to_hsv (h0,s0,v0,&h0, &s0, &v0);
      _rgb_to_hsv (h1,s1,v1, &h1, &s1, &v1);
		       ...
		       gboolean    select)
{
  ...
  if (gradient->pixmap == NULL)
    {
      gradient->pixmap =
		  gdk_pixmap_new (shell->window, 48, 16, -1);
    }
